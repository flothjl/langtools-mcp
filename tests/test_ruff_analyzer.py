...
from langtools_mcp.lsp.ruff_analyzer import RuffAnalyzer
...
def test_ruff_analyzer_clean(monkeypatch):
    ...
    monkeypatch.setattr("langtools_mcp.lsp.lsp_daemon_client.LSPDaemonClient.analyze", lambda self, f, l: resp)
...
def test_ruff_analyzer_error(monkeypatch):
    ...
    monkeypatch.setattr("langtools_mcp.lsp.lsp_daemon_client.LSPDaemonClient.analyze", lambda self, f, l: err_resp)
    analyzer = RuffAnalyzer()
    with tempfile.NamedTemporaryFile(suffix='.py', delete=False) as tf:
        result = analyzer.analyze(tf.name)
